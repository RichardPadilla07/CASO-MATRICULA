...
async function crearPedido(e) {
  ...
  try {
    ...
    const resEst = await fetch(`https://caso-matricula.onrender.com/api/estudiantes/${cedula}`);
    if (!resEst.ok) throw new Error('No se encontró el estudiante');
    const estudiante = await resEst.json();
    // Buscar materia guardada por código de matrícula
    const resMaterias = await fetch(`https://caso-matricula.onrender.com/api/materiasGuardadas/${cedula}`);
    ...
    const resMat = await fetch(`https://caso-matricula.onrender.com/api/materias/codigo/${materia.codigo_materia}`);
    ...
    const res = await fetch('https://caso-matricula.onrender.com/api/matriculas', {
      method: 'POST',
...
async function mostrarPedidosCliente() {
  ...
  try {
    ...
    const res = await fetch(`https://caso-matricula.onrender.com/api/matriculas/estudiante/${cedula}`);
...
function renderPedidosTabla(matriculas) {
  ...
  <td>${m.id_estudiante && m.id_estudiante.cedula ? m.id_estudiante.cedula : ''}</td>
